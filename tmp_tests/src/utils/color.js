...
export function solidPaintToRGB(paint) {
...
/** 塗りから最初のSolid色を取得 */
export function firstSolidFromFills(node) {
    if ('fills' in node) {
        const fills = node.fills;
        if (fills === figma.mixed || !fills || !Array.isArray(fills))
            return null;
...
export function estimateBackgroundRGB(node) {
        ...
        if (hasFills(current)) {
            ...
            if (fills !== figma.mixed && Array.isArray(fills)) {
                ...
                if (solidPaint) {
                    ...
                    if (rgb)
                        return { rgb, source: `parent:${current.name}` };
                }
            }
        }
